...
function looksLikeUserRecord(o: Record<string, unknown>): boolean {
  ...
  const hasContact =
    typeof o.email === "string" || typeof o.phone === "string" || typeof o.name === "string";
  const namedEmail = typeof o.name === "string" && (typeof o.email === "string" || typeof o.phone === "string");
  return (idOk && hasContact) || namedEmail;
}

export async function fetchProfile(config?: RequestConfig): Promise<ProfilePayload> {
  const res = await apiClient.get<any>(adminAuthRoutes.profile, config);
