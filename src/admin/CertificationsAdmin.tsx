...
} from "@/services/admin-service/certificate/certificate.service";
...
const CertificationsAdmin = () => {
  ...
  const [newForm, setNewForm] = useState<CertificationForm>(emptyForm);
  ...
  const [editingForm, setEditingForm] = useState<CertificationForm>(emptyForm);
  ...
  const [itemToDelete, setItemToDelete] = useState<CertificationItem | null>(null);
  ...
  const loadItems = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchCertificationsPage({ page, limit: PAGE_SIZE }, { suppress401Redirect: true });
      setItems(res.list);
      setTotal(res.total);
    } catch (err) {
  ...
  const openView = (item: CertificationItem) => {
    setViewingItem(item);
    setIsViewOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCertification(target.id, { suppress401Redirect: true });
            ...
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground font-body">
                  API content from `/admin/about/certifications`.
                </p>
              </div>
              <Dialog
                open={isAddOpen}
              ...
              >
                ...
                <DialogContent className="max-w-xl">
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        placeholder="Responsibly sourced wood"
                        rows={3}
                      />
                    </div>
                  </div>
                  <DialogFooter>
                    ...
                    >
                      {creating ? "Saving..." : "Save Certification"}
                    </button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="p-0">
                        <div className="flex min-h-[180px] items-center justify-center py-8">
                          <Loading size={24} className="text-primary" message="Loading certifications..." />
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : items.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={4} className="p-0">
                        <Empty
                          ...
                          description="No record was returned from /admin/about/certifications."
                        />
                      </TableCell>
                    </TableRow>
                  ) : (
                      ...
                      <div className="relative bg-gradient-to-b from-muted/60 via-muted/30 to-background px-6 pb-6 pt-8">
                        ...
                        <DialogHeader className="relative space-y-0 text-center sm:text-center">
                          ...
                          <DialogTitle className="mt-5 px-1 font-display text-xl font-semibold leading-snug tracking-tight text-foreground">
                            {viewingItem.title || "Untitled certification"}
                          </DialogTitle>
                          <DialogDescription className="mx-auto max-w-[320px] pt-1.5 text-xs font-body text-muted-foreground">
                            Certifications · public-facing content for the About page
                          </DialogDescription>
                        </DialogHeader>
                      </div>
