...
const defaultItems: ValueItem[] = [
  {
    icon: "",
    title: "Craftsmanship",
    description: "Every piece is made by skilled artisans using traditional joinery techniques passed down through generations.",
  },
  {
    icon: "",
    title: "Sustainability",
    description: "We use only FSC-certified wood and eco-friendly finishes. Our factories are powered by 100% renewable energy.",
  },
  {
    icon: "",
    title: "Community",
    description: "We partner with local workshops across Scandinavia, supporting small communities and preserving craft traditions.",
  },
];

function safeParseValues(raw: string): ValueItem[] | null {
  try {
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) return null;
    const normalized: ValueItem[] = parsed.map((t: any) => {
      return {
...
const ValuesAdmin = () => {
  const [items, setItems] = useState<ValueItem[]>(defaultItems);
  ...
  const [newItem, setNewItem] = useState<ValueItem>({
    icon: "",
    title: "",
    description: "",
  });
  ...
  const [viewingItem, setViewingItem] = useState<ValueItem | null>(null);

  useEffect(() => {
    const overrideItemsRaw = getOverride("about.values.items", "");
    const parsed = overrideItemsRaw ? safeParseValues(overrideItemsRaw) : null;
  ...
  const save = () => {
    setSaving(true);
    setOverrides({
      "about.values.items": JSON.stringify(items),
    });
    setTimeout(() => {
      setSaving(false);
      toast.success("Our Values items updated");
    }, 600);
  };
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground font-body">Manage the company values section.</p>
              </div>
